...
void FVoxelMembersSchemaAction_Function::BuildContextMenu(FMenuBuilder& MenuBuilder)
{
	...
	if (!ensure(TerminalGraph) ||
		TerminalGraph->GetBaseTerminalGraphs().Num() < 2)
	{
		return;
	}
	...
	MenuBuilder.AddMenuEntry(
		...
		FUIAction
		{
			MakeLambdaDelegate([=, this]
			{
				...
				const TVoxelArray<const UVoxelTerminalGraph*> BaseTerminalGraphs = LocalTerminalGraph->GetBaseTerminalGraphs().Array();
				check(BaseTerminalGraphs[0] == LocalTerminalGraph);
...
void FVoxelMembersSchemaAction_Function::OnDelete() const
{
	...
	TVoxelArray<UVoxelGraphNode_CallMemberFunction*> FunctionNodes;
	if (TerminalGraph->IsTopmostTerminalGraph())
	{
		const FGuid Guid = TerminalGraph->GetGuid();

		ForEachObjectOfClass<UVoxelGraphNode_CallMemberFunction>([&](UVoxelGraphNode_CallMemberFunction& FunctionNode)
		{
...

	{
		...
		{
			for (UVoxelGraphNode_CallMemberFunction* Node : FunctionNodes)
			{
		...
		[
			...
			[
				...
				.Visibility_Lambda([this]
				{
					...
					const UVoxelTerminalGraph* TerminalGraph = static_cast<FVoxelMembersSchemaAction_Function&>(*Action).WeakTerminalGraph.Resolve();
					if (!TerminalGraph)
					{
						return EVisibility::Collapsed;
					}
	...
	UEdGraph& EdGraph)
{
	...
	Action.Apply(EdGraph, UE_506_SWITCH(FVector2D, FVector2f)(GraphPosition), true);

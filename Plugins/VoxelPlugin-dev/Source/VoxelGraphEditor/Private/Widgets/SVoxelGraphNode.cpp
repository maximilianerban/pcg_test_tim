...
TArray<FOverlayWidgetInfo> SVoxelGraphNode::GetOverlayWidgets(bool bSelected, const UE_506_SWITCH(FVector2D, FVector2f)& WidgetSize) const
{
	...
	for (const auto& OverlayWidget : OverlayWidgets)
	{
		...
		Info.OverlayOffset = OverlayWidget.GetLocation(FVector2D(WidgetSize), Offset);
...
void SVoxelGraphNode::GetOverlayBrushes(bool bSelected, UE_506_SWITCH(const FVector2D, const FVector2f&) WidgetSize, TArray<FOverlayBrushInfo>& Brushes) const
{
...
FReply SVoxelGraphNode::OnMouseButtonDoubleClick(const FGeometry& InMyGeometry, const FPointerEvent& InMouseEvent)
{
	if (bBlockDoubleClick)
	{
		return FReply::Unhandled();
	}

	return SVoxelGraphNodeBase::OnMouseButtonDoubleClick(InMyGeometry, InMouseEvent);
}

void SVoxelGraphNode::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	SVoxelGraphNodeBase::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);

	if (bBlockDoubleClick)
	{
		bBlockDoubleClick = false;
	}
}
...
void SVoxelGraphNode::UpdateStandardNode()
{
...
#else
	.SlotOffset2f(MakeAttributeSP(&CommentBubble.Get(), &SCommentBubble::GetOffset2f))
	.SlotSize2f(MakeAttributeSP(&CommentBubble.Get(), &SCommentBubble::GetSize2f))
#endif
	...
	// Empty pin labels should not take up space in PinWidget
	for (const TSharedRef<SGraphPin>& PinWidget : InputPins)
	{
		...
		const FString Name = PinObject->GetDisplayName().ToString().TrimStartAndEnd();
		if (Name.IsEmpty())
		{
	...
	const TSharedPtr<SVerticalBox>& TargetContainer)
{
	...
	PinWidget->SetVisibility(MakeAttributeLambda([=, this]
	{
		if (Pin->bHidden ||
	...
	const bool bIsInput)
{
	...
	const TSharedRef<SVerticalBox> Result =
		...
		[
			...
			[
				...
				{
					const FName Category = Node->GetConcatenatedPath();
					if (Category.ToString().StartsWith("__collapsed__"))
					{
						TSet<FName>& ExpandedAdvancedCategories = bIsInput ? GetVoxelNode().ExpandedInputAdvancedCategories : GetVoxelNode().ExpandedOutputAdvancedCategories;
						if (ExpandedAdvancedCategories.Contains(Category))
						{
							ExpandedAdvancedCategories.Remove(Category);
						}
						else
						{
							ExpandedAdvancedCategories.Add(Category);
						}
				...
				[
					...
					[
						...
						.Text(INLINE_LAMBDA -> FText
						{
							FString Name = Node->Name.ToString();
							Name.RemoveFromStart("__collapsed__");
	...
	[
		...
		.Image_Lambda([=, this]() -> const FSlateBrush*
		{
			...
			if (Category.ToString().StartsWith("__collapsed__"))
			{
				const TSet<FName>& ExpandedCategories = bIsInput ? GetVoxelNode().ExpandedInputAdvancedCategories : GetVoxelNode().ExpandedOutputAdvancedCategories;
				bIsExpanded = ExpandedCategories.Contains(Category);
			}
			else
			{
				...
				bIsExpanded = !CollapsedCategories.Contains(Category);
			}
...
TSharedRef<SWidget> SVoxelGraphNode::MakeStatWidget() const
{
	...
	const auto AddStatWidget = [&](IVoxelNodeStatProvider* Provider, UEdGraphPin* Pin)
	{
		...
		if (Pin)
		{
			Color = MakeAttributeLambda([=, this]() -> FSlateColor
			{
				const TSharedPtr<SGraphPin> Widget = FindWidgetForPin(Pin);
				if (!ensureVoxelSlow(Widget))
				{
					return FColor::Orange;
				}

				return FLinearColor(FColor::Orange) * (Widget->IsHovered() ? 1.f : 0.6f);
			});
		}
		else
		{
...
void SVoxelGraphNode::CreateStandardNodeTitleButtons(const TSharedPtr<SHorizontalBox>& TitleBox)
{
	...
	for (const FProperty& Property : GetStructProperties(StructNode->Struct->GetStruct()))
	{
		if (!Property.HasAnyPropertyFlags(CPF_Edit) ||
			Property.GetFName() == GET_MEMBER_NAME_CHECKED(FVoxelNode, NodeGuid) ||

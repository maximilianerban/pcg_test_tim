...
bool UVoxelGraphNode::IsVisible(const IVoxelNodeDefinition::FNode& Node) const
{
	...
	{
		const TSet<FName>& ExpandedCategories = Node.IsInput() ? ExpandedInputAdvancedCategories : ExpandedOutputAdvancedCategories;
...
bool UVoxelGraphNode::CanSplitPin(const UEdGraphPin& Pin) const
{
	...
	{
		return GVoxelNodeLibrary->FindMakeNode(Pin.PinType) != nullptr;
	}
	else
	{
		ensure(Pin.Direction == EGPD_Output);
		return GVoxelNodeLibrary->FindBreakNode(Pin.PinType) != nullptr;
	}
}
...
void UVoxelGraphNode::SplitPin(UEdGraphPin& Pin)
{
	...
	const TSharedPtr<const FVoxelNode> NodeTemplate = Pin.Direction == EGPD_Input
			? GVoxelNodeLibrary->FindMakeNode(Pin.PinType)
			: GVoxelNodeLibrary->FindBreakNode(Pin.PinType);
...
void UVoxelGraphNode::OnRenamePinRequest_ContextMenu(const UEdGraphPin& Pin)
{
	const FSimpleDelegate* Delegate = PinToRenameDelegate.Find(Pin.PinName);
	if (!ensure(Delegate))
	{
		return;
	}
...
void UVoxelGraphNode::AddPinToRename(const FName PinName, FSimpleDelegate&& Delegate)
{
	PinToRenameDelegate.FindOrAdd(PinName) = Delegate;
}
...
void UVoxelGraphNode::AllocateDefaultPins()
{
	...
	ExpandedInputAdvancedCategories = ExpandedInputAdvancedCategories.Intersect(ValidCategories);
	ExpandedOutputAdvancedCategories = ExpandedOutputAdvancedCategories.Intersect(ValidCategories);
...
void UVoxelGraphNode::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
void UVoxelGraphNode::InitializeDefaultValue(
	FVoxelNode& VoxelNode,
	const FVoxelPin& VoxelPin,
	UEdGraphPin& GraphPin) const
{
	VOXEL_FUNCTION_COUNTER();
	...
	FString DefaultValueString = VoxelNode.GetPinDefaultValue(VoxelPin);
	...
	if (InnerExposedType.Is<FVoxelExposedSeed>())
	{
		...
		VoxelNode.UpdatePropertyBoundDefaultValue(VoxelPin, DefaultValue);

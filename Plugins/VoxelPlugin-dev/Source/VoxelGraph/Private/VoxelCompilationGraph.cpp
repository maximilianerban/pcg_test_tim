...
{
	ensure(GActivePinViews.Pop(EAllowShrinking::No) == Pins);
}
...
void FNode::ApplyErrorsTo(FNode& Other) const
{
	Other.Errors = Errors;
}
...
FNode& FGraph::NewNode(const ENodeType Type, const FVoxelGraphNodeRef& NodeRef)
{
	...
	FNode& Node = NodeAllocations.Emplace_GetRef(
		FNode::EPrivate{},
		Type,
		NodeRef,
		*this);
...
bool FGraph::Identical(const FGraph& Other, FString* OutDiff) const
{
	...
	for (const FNode& Node : GetNodes())
	{
		ensure(Node.GetVoxelNode().HasEditorData());
	}

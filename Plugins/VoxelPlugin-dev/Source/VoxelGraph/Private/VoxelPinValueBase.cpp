...
bool FVoxelPinValueBase::ImportFromString(const FString& Value)
{
	...
	case EVoxelPinInternalType::UInt16:
	{
		UInt16 = FVoxelUtilities::ClampToUINT16(FCString::Atoi(*Value));
		return true;
	}
	case EVoxelPinInternalType::Int32:
	{
...
#define CHECK(InType, ...) \
		...
		if (Type.Is<FVector>())
		{
			...
			CHECK(FVector, FVector::ForwardVector);
			CHECK(FVector, FVector::RightVector);
			CHECK(FVector, FVector::UpVector);

			CHECK(FVector, FVector3f::ZeroVector);
			CHECK(FVector, FVector3f::OneVector);
			CHECK(FVector, FVector3f::ForwardVector);
			CHECK(FVector, FVector3f::RightVector);
			CHECK(FVector, FVector3f::UpVector);
			...
			if (FVoxelUtilities::IsFloat(Value))
			{
				Get<FVector>() = FVector(FVoxelUtilities::StringToFloat(Value));
				return true;
			}
		...
		else if (Type.Is<FVector2D>())
		{
			...
			if (FVoxelUtilities::IsFloat(Value))
			{
				Get<FVector2D>() = FVector2D(FVoxelUtilities::StringToFloat(Value));
				return true;
			}
		...
		else if (Type.Is<FVoxelDoubleVector>())
		{
			if (FVoxelUtilities::IsFloat(Value))
			{
				Get<FVoxelDoubleVector>() = FVector(FVoxelUtilities::StringToFloat(Value));
				return true;
			}
		...
		else if (Type.Is<FVoxelDoubleVector2D>())
		{
			if (FVoxelUtilities::IsFloat(Value))
			{
				Get<FVoxelDoubleVector2D>() = FVector2D(FVoxelUtilities::StringToFloat(Value));
				return true;
			}
...
void FVoxelPinValueBase::Fixup()
{
	...
	if (const TSharedPtr<FVoxelPinValueOps> Ops = FVoxelPinValueOps::Find(GetType(), EVoxelPinValueOpsUsage::Fixup))
	{
		Ops->Fixup(*this);
	}
}

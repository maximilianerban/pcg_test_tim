...
private:
	void Build(const FVoxelBuffer& Buffer, const FName Type)
	{
		...
		for (const FProperty& Property : GetStructProperties(Buffer.GetStruct()))
		{
			...
			if (const FVoxelTerminalBuffer* TerminalBuffer = ChildBuffer->As<FVoxelTerminalBuffer>())
			{
	...
	void ParseRange(const FVoxelTerminalBuffer& Buffer, const FName Type)
	{
		if (const FVoxelFloatBuffer* FloatBuffer = Buffer.As<FVoxelFloatBuffer>())
		{
			FloatRanges.Add_CheckNew(Type, FVoxelUtilities::GetMinMaxSafe(FloatBuffer->View()));
			Order.Add(Type);
		}
		else if (const FVoxelDoubleBuffer* DoubleBuffer = Buffer.As<FVoxelDoubleBuffer>())
		{
			DoubleRanges.Add_CheckNew(Type, FVoxelUtilities::GetMinMaxSafe(DoubleBuffer->View()));
			Order.Add(Type);
		}
		else if (const FVoxelInt32Buffer* Int32Buffer = Buffer.As<FVoxelInt32Buffer>())
		{
			Int32Ranges.Add_CheckNew(Type, FVoxelUtilities::GetMinMax(Int32Buffer->View()));
			Order.Add(Type);
		}
	}
};
...
public:
	...
	TVoxelMap<TVoxelObjectPtr<const UEdGraphNode>, TVoxelMap<FEdGraphPinReference, TSharedPtr<FVoxelRangeStats>>> NodeToPinToStats;
...
void FVoxelNode_RangeDebug::Compute(const FVoxelGraphQuery Query) const
{
	ensure(GIsEditor);
	...
	VOXEL_GRAPH_WAIT(Value)
	{
		...
		if (Value.IsBuffer())
		{
			GVoxelNodeRangeStatManager->EnqueueStats(*this, RefPin, MakeShared<FVoxelRangeStats>(Value.Get<FVoxelBuffer>(), ""));
		}
#endif
